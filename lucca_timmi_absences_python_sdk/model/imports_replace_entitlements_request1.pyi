# coding: utf-8

"""
    Timmi Absences API

    Welcome on the documentation for Timmi Absences API.

    The version of the OpenAPI document: 1.0
    Contact: developers@lucca.fr
    Created by: https://www.lucca.fr
"""

from datetime import date, datetime  # noqa: F401
import decimal  # noqa: F401
import functools  # noqa: F401
import io  # noqa: F401
import re  # noqa: F401
import typing  # noqa: F401
import typing_extensions  # noqa: F401
import uuid  # noqa: F401

import frozendict  # noqa: F401

from lucca_timmi_absences_python_sdk import schemas  # noqa: F401


class ImportsReplaceEntitlementsRequest1(
    schemas.DictSchema
):
    """
    This class is auto generated by Konfig (https://konfigthis.com)
    """


    class MetaOapg:
        
        class properties:
            File = schemas.StrSchema
            Description = schemas.StrSchema
            ReferenceDate = schemas.DateTimeSchema
            EntryTypes = schemas.StrSchema
            __annotations__ = {
                "File": File,
                "Description": Description,
                "ReferenceDate": ReferenceDate,
                "EntryTypes": EntryTypes,
            }
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["File"]) -> MetaOapg.properties.File: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["Description"]) -> MetaOapg.properties.Description: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["ReferenceDate"]) -> MetaOapg.properties.ReferenceDate: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["EntryTypes"]) -> MetaOapg.properties.EntryTypes: ...
    
    @typing.overload
    def __getitem__(self, name: str) -> schemas.UnsetAnyTypeSchema: ...
    
    def __getitem__(self, name: typing.Union[typing_extensions.Literal["File", "Description", "ReferenceDate", "EntryTypes", ], str]):
        # dict_instance[name] accessor
        return super().__getitem__(name)
    
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["File"]) -> typing.Union[MetaOapg.properties.File, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["Description"]) -> typing.Union[MetaOapg.properties.Description, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["ReferenceDate"]) -> typing.Union[MetaOapg.properties.ReferenceDate, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["EntryTypes"]) -> typing.Union[MetaOapg.properties.EntryTypes, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: str) -> typing.Union[schemas.UnsetAnyTypeSchema, schemas.Unset]: ...
    
    def get_item_oapg(self, name: typing.Union[typing_extensions.Literal["File", "Description", "ReferenceDate", "EntryTypes", ], str]):
        return super().get_item_oapg(name)
    

    def __new__(
        cls,
        *args: typing.Union[dict, frozendict.frozendict, ],
        File: typing.Union[MetaOapg.properties.File, str, schemas.Unset] = schemas.unset,
        Description: typing.Union[MetaOapg.properties.Description, str, schemas.Unset] = schemas.unset,
        ReferenceDate: typing.Union[MetaOapg.properties.ReferenceDate, str, datetime, schemas.Unset] = schemas.unset,
        EntryTypes: typing.Union[MetaOapg.properties.EntryTypes, str, schemas.Unset] = schemas.unset,
        _configuration: typing.Optional[schemas.Configuration] = None,
        **kwargs: typing.Union[schemas.AnyTypeSchema, dict, frozendict.frozendict, str, date, datetime, uuid.UUID, int, float, decimal.Decimal, None, list, tuple, bytes],
    ) -> 'ImportsReplaceEntitlementsRequest1':
        return super().__new__(
            cls,
            *args,
            File=File,
            Description=Description,
            ReferenceDate=ReferenceDate,
            EntryTypes=EntryTypes,
            _configuration=_configuration,
            **kwargs,
        )
